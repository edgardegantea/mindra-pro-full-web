...
class InferenceService
{
    ...
    public function predict(?User $user = null, ?UploadedFile $audio = null, string $text = '', ?UploadedFile $image = null, ?float $durationSeconds = null): array
    {
        $plan = $this->resolvePlan($user);

        if ($plan->slug === Plan::FREE && $audio && $image) {
            return ['ok' => false, 'error' => 'El plan Free sólo admite texto y audio, no imágenes.'];
        }

        if ($plan->slug === Plan::FREE && $image) {
        ...
        $result = $this->mindraback->predict($audio, $text, $image);

        if (!$result['ok']) {
            return $result;
        }

        return DB::transaction(function () use ($user, $audio, $text, $durationSeconds, $image, $plan, $result) {
