...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:32px;">
    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:16px;padding:24px;">
        ...
        @php
            $planColors = ['free' => ['#e2e8f0','#64748b'], 'pro' => ['#c7d2fe','#4f46e5'], 'plus' => ['#ddd6fe','#7c3aed'], 'full' => ['#ddd6fe','#7c3aed']];
            $planLabels = ['free' => 'Free', 'pro' => 'Pro', 'plus' => 'Full', 'full' => 'Full'];
            $planTotal = $planDistribution->sum() ?: 1;
        @endphp
        <div style="display:flex;flex-direction:column;gap:14px;">
            @foreach(['free','pro','plus'] as $slug)
                @php
                    $count = $planDistribution->get($slug, 0) + ($slug === 'plus' ? $planDistribution->get('full', 0) : 0);
